...
규모와 상태는 `node tools/counts.mjs` 가 찍어 줍니다 — 여기 적지 않습니다.
(적혀 있던 「프리셋 연결 358개」·「`conventional` 237」은 2026-09-20 에 재어 보니
각각 355 와 441 이었습니다. 수를 문서에 박아 두면 이렇게 낡습니다.)

> **낡았다는 인상만큼 낡지는 않았습니다.** `updated` 는 2026-08-01 에 멈춰 있지만,
> 2026-09-20 에 재어 보니 **프리셋 연결이 한 건도 끊기지 않았습니다** — 연결된
> 이름이 전부 실재하는 프리셋입니다. 연결되지 않은 프리셋 둘은 `Play Along` ·
> `Casio Cells` 로, 장르가 아닌 데모라 **빠진 것이 맞습니다.**
>
> 다만 그 «끊기지 않음» 을 지켜 주는 것이 여태 없었습니다. 프리셋 이름을 바꾸면
> 조용히 끊깁니다. 이제 `node tools/counts.mjs` 가 끊긴 연결 수를 함께 찍습니다.
> ⚠ 그래도 **내용**(bpm · feel · tags)이 문서와 어긋나는 것은 아무도 검사하지
> 않습니다. 연결이 살아 있다고 값이 맞다는 뜻은 아닙니다.
