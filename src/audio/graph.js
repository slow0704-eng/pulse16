...
function boot(){
  ...
  TUBE={cln:mkTube(1.15,0.06), cru:mkTube(2.4,0.22),
        ...
        /* 퍼즈는 진공관이 아니라 트랜지스터를 한계까지 밀어 거의 사각파를
           ...
           ⚠ fz1 의 bias 는 0 이어야 합니다. bias 는 대칭을 깨서 짝수 배음을
              만드는 장치인데, 퍼즈의 정체성은 **홀수 배음 우세**입니다.
              bias 0.20 이던 시절 홀/짝이 0.2dB 로 측정됐습니다(기준 +4~+12). */
        /* 4단 클리핑 실험(과제②, tools/측정결과-신규음색.md 미해결분) — 해 봤지만
           버렸다. 빅머프처럼 fz1 뒤에 대칭(bias 0) 클리핑을 하나 더 끼워
           3단→4단으로 늘렸더니(단간에는 하이패스 80Hz 커플링만) 홀/짝비가
           +1.6~+2.0dB → −4.8~−5.5dB 로 **더 나빠졌다**(tools/measure-body.html
           실측). 대칭 클리핑을 하나 더 얹는다고 홀수 배음이 더 세지는 게
           아니라, 이미 홀·짝이 섞여 있는(현 자체가 +2.6dB뿐) 신호를 한 번
           더 비선형으로 굴려 상호변조를 늘려 오히려 비율이 흐려진 것으로
           보인다. 정직하게 원상복구했다 — 3단 유지. */
        fz1:mkTube(9.0,0.0), fz2:mkTube(14.0,0.0), fz3:mkTube(7.0,0.0)};
  ...
  /* ── 밴조 몸통 — 병렬 모달 뱅크 (docs/mutable-차용.md ★2) ──
     위 body_banjo(직렬 peaking)는 지우지 않고 남겨 둔다 — 대조 측정에 필요하고,
     GTR.banjo.amp 는 다른 담당자 소유(core/engines.js)라 여기서 스위치를
     넣지 않는다. 갈아타려면 그 파일에서 amp:'body_banjo' → 'body_banjo_modal' 로
     바꾸면 된다(보고서에 제안 남김).

     모드 표는 실물 밴조 가죽 헤드를 참고해 손으로 잡았다 — 막(膜) 진동이라
     정수배 배음이 아니고(370 · 520 은 2:2.8 정도), 헤드는 손·브리지에 눌려
     감쇠가 빠르다(T60 0.06~0.22s, 통기타 몸통보다 훨씬 짧다). 브리지·톤링이
     만드는 3kHz 대역의 쇳소리는 더 짧게(0.06s) 잡아 "쨍"하고 바로 죽게 했다.
     트림 −9dB 는 1차 추정값 — tools/measure-body.html 로 실측해 맞춘다. */
  const BANJO_MODAL_MODES = [
    [ 370, 0.22, 1.00],   // 헤드 기본 모드 — 밴조의 정체성, 픽이 때리면 "봉" 하고 받힌다
    [ 520, 0.11, 0.35],   // 헤드 배음(비조화 — 정수배 아님)
    [1200, 0.09, 0.45],   // 브리지 대역
    [3000, 0.06, 0.55],   // 톤링 쇳소리
  ];
  const banjoModalPre = wire([bqf('highpass',160,0.7), bqf('highpass',160,0.7)]);
  const banjoBank = modalBank(BANJO_MODAL_MODES,
    wire([bqf('lowpass',9000,0.707), chan.gtr]), {dry:0.35, trim:-10.4});
  /* 트림 −10.4dB : tools/measure-body.html 로 실측해 맞춘 값(RMS차 +1.4dB → −10.4
     로 내리니 목표 ±1.0dB 안에 들어옴). 고Q 병렬 뱅크가 레벨을 올리는 것을
     이렇게 실측으로 보정한다 — 과거 −7.9 LUFS 사고를 반복하지 않기 위함. */
  banjoModalPre.connect(banjoBank);
  banjoModalPre._modalBank = banjoBank._modalBank;   // 계측 하네스가 모드별 게인을 찾을 수 있게 (ampIn.*_modal 은 프리필터 노드라 원래 자리엔 없다)
  ampIn.body_banjo_modal = banjoModalPre;

  /* 퍼즈 페달 → 앰프.
     하이게인(amp_hi)과 다른 점을 셋으로 만듭니다.
     ① 페달 앞에서 저역을 크게 잘라냄(160Hz ×2) — 퍼즈가 저역을 못 버팀
     ② 중역을 파냄(600Hz −7dB) — 그런지의 그 빈 중역
     ③ 클리핑을 세 번, 사이에 필터를 거의 안 둠 — 배음이 정연하지 않게
       (4단 실험은 해 봤지만 버렸다 — TUBE 정의 옆 주석 참고)

     ── 과제② 서스테인 폭주 (tools/측정결과-신규음색.md 미해결분) ──
     기음 T60 18.8~22.3s(설계 5.04)에 1.0초 지점 레벨이 +1.1dB 로 오르는 것이
     미해결로 남아 있었다. 3단째 클리핑을 빼도 그대로였다는 것까지 확인돼
     있었다 — "마지막 단" 문제가 아니라 "얼마나 깊게 클리핑하는가" 문제다.
     드라이브 게인 9.0(≈+19dB)은 A2 를 친 순간부터 한참(초 단위) 동안 신호가
     클리퍼 문턱을 훨씬 웃돌게 만든다 — 이 구간에서는 입력이 더 커도 출력은
     거의 그대로(압축비→무한대)라, 현이 자연 감쇠하는 동안 출력 레벨이 거의 안
     떨어지다가(그래서 "폭주"처럼 보인다) 문턱 근처에 와서야 감쇠가 드러난다.
     "느린 감쇠 엔벨로프를 얹는" 땜질 대신, 게인 자체를 낮춰 문턱을 웃도는
     시간을 줄인다 — 압축비가 낮아지면 자연 감쇠가 더 일찍 드러난다.
     9.0 → 4.0 으로 낮추고 뒤에서 게인을 보충한다(gnn(0.09)→gnn(0.22)).
     tools/measure-body.html 실측(0.05s=0dB 기준, 여러 번 재서 범위로 적음):
       고치기 전  0.5s +0.2~0.6  1.0s −0.0~+1.0  2.0s +0.2~0.9dB (뚜렷이 오름)
       고친 후    0.5s +0.2~0.6  1.0s −0.0~+0.5  2.0s +0.3~0.5dB (거의 평탄, 뚜렷한 상승 없음)
     완전한 "자연 감쇠처럼 떨어짐" 까지는 못 갔다 — 여전히 0dB 부근에 오래
     머문다(광대역 T60 추정 9~11s, 설계 5.04s). 압축비를 더 낮추려면 드라이브를
     더 내려야 하는데, 그러면 클리핑이 얕아져 퍼즈의 정체성(홀수 배음·낮은
     크레스트)이 옅어진다 — 정직하게 "개선됐지만 미해결" 로 남긴다. */
  gtrDrv.fz = gnn(4.0);
  /* 하이패스 160Hz 를 두 번 걸면 A2(110Hz) 기음이 −19.3dB 로 사라진다 —
     amp_hi 가 150→100 으로 이미 고쳤던 것과 같은 버그다.(측정 확인)
     퍼즈는 "저역이 얇아진다" 가 특징이지 "기음이 없다" 가 아니다. */
  ampIn.amp_fuzz = wire([
    bqf('highpass',100,0.7), bqf('highpass',100,0.7),
    bqf('peaking',1600,1.2,3.0),          // 입력 프레즌스 — 클리핑에 밝기를 준다
    gtrDrv.fz, shp(TUBE.fz1),
    bqf('lowpass',7000,0.707),
    gnn(3.2), shp(TUBE.fz2),
    /* ⚠ 중역 스쿱은 왜곡 **뒤**에 와야 합니다. 앞에 두면 클리핑이 중역을
       다시 만들어 내서 소용이 없습니다 — 파여야 할 600Hz 가 clean 대비
       +15.8dB 로 솟아 있었습니다.(측정) 빅머프의 톤 스택도 왜곡 뒤에 있습니다. */
    bqf('peaking',600,1.1,-9.0), bqf('peaking',900,1.4,-4.0),
    bqf('peaking',2600,1.2,6.0),          // 하이 프레즌스 — 중심 주파수를 올린다
    /* 3단째 클리핑 — 톤 스택 **뒤**입니다. 두 가지를 한 번에 합니다.
       ① 스쿱으로 만든 홀수 배음이 다시 깎이지 않습니다(앞에 두면 깎입니다)
       ② 첨두를 마지막에 눌러 크레스트를 내립니다 — EQ 를 클리핑 뒤에 두면
          RMS 만 깎여 크레스트가 오히려 올라갑니다.
       빼고 재 봤더니 크레스트 14.2→18.3, 중심 797→410Hz 로 되돌아갔습니다.
       (서스테인 폭주는 이 단 때문이 아닙니다 — 빼도 18.8초 그대로였습니다) */
    gnn(1.6), shp(TUBE.fz3),
    bqf('highpass',110,0.7), gnn(0.22),   // 드라이브를 낮춘 만큼(9.0→4.0) 출력 트림을 보충
    ...cab(6200,90,-6.0,4.5,1.6), chan.gtr]);
  ...
  /* ── 시타르 몸통 — 병렬 모달 뱅크 ──
     박(tumba)은 밴조 헤드보다 훨씬 크고 깊은 통이라 기본 공진이 오래 간다
     (T60 0.35s — 밴조 헤드의 0.22s보다 길게). 자와리 버즈가 걸리는 2~4kHz
     대역은 string.js 의 buzz(★3 대상)가 이미 담당하므로 여기서는 그
     대역의 "통 공명" 만 짧게 보탠다(T60 0.05~0.08s). body_sitar(직렬)는
     대조 측정을 위해 남겨 둔다. GTR.sitar.amp 스위치는 다른 담당자 소유
     (core/engines.js) — 보고서에 제안만 남긴다. */
  const SITAR_MODAL_MODES = [
    [ 180, 0.35, 1.00],   // 박 기본 공진 — 크고 빈 통이라 가장 오래 운다
    [ 420, 0.19, 0.40],   // tools/measure-body.html 실측 보정: 0.15 로 두면 실측 T60 이
                           // 0.118s(−21%, 판정 밖)로 나와 0.19 로 올렸다 — 이 Q 대역(27~30)에서
                           // 왜 체계적으로 짧게 나오는지는 못 밝혔다(정직 원칙 — 보고서 참고).
    [2600, 0.08, 0.55],   // 자와리 버즈 대역과 겹치는 브라이트니스
    [4200, 0.05, 0.35],
  ];
  const sitarModalPre = wire([bqf('highpass',110,0.7)]);
  const sitarBank = modalBank(SITAR_MODAL_MODES,
    wire([bqf('lowpass',8000,0.707), chan.gtr]), {dry:0.30, trim:-6.0});
  /* 트림 −6.0dB : 실측으로 맞춘 값(RMS차 −2.1dB → −6.0 로 올리니 목표 안). */
  sitarModalPre.connect(sitarBank);
  sitarModalPre._modalBank = sitarBank._modalBank;
  ampIn.body_sitar_modal = sitarModalPre;

  /* ── 베이스 앰프 · 업라이트 몸통 ──
     현 베이스 엔진은 합성 베이스와 달리 서브 오실레이터가 없다.
     실제 베이스 앰프처럼 80Hz 를 세우고 400Hz 를 파서 자리를 만든다.

     ── 2026-08-17 : 로우셸프와 메이크업 게인을 넣었다 ──
     여기에는 **출력 메이크업이 아예 없었다.** amp_cln·amp_hi·amp_fuzz 는
     전부 gnn() 으로 끝나는데 bass_amp·bass_upright 만 없었다.
     그래서 현 베이스는 합성 베이스보다 10.6dB 작게 나갔다 —
       합성: pS 0.484 + pH 1.531(HMAKE ×4.2) → 피크 −0.5 dBFS
       현  : 0.9 × lvl 0.40(pick)             → 피크 −11.1 dBFS
     커밋 7a79ab3 의 배음 메이크업(×4.2)이 합성 분기에만 걸렸고
     BSTR 11종은 통째로 빠져 있었기 때문이다.

     실측(록 프리셋, 20~120Hz): 베이스가 킥보다 18.7~25.0dB 아래.
     같은 자리에서 합성 엔진을 쓰는 힙합은 −0.6~+4.4dB 다.
     80Hz 벨(Q1.2)만으로는 기음 하나를 못 세워서, 120Hz 아래를
     통째로 드는 로우셸프로 바꿔 «지속하는 무게» 를 만든다.

     ⚠ 셸프는 저역만 든다. 메이크업 gnn 은 전대역이라 크레스트가 큰
       현 베이스(25dB)의 첨두가 그대로 올라간다 — 값을 키울 때는
       반드시 마스터 트루피크를 다시 재야 한다. */
  ampIn.bass_amp = wire([bqf('highpass',30,0.7),
    bqf('lowshelf',130,0.7,12.0),
    bqf('peaking',80,1.2,3.5), bqf('peaking',400,1.0,-2.5),
    bqf('peaking',800,1.2,1.5), bqf('peaking',2500,1.1,2.5),
    bqf('lowpass',5000,0.707), gnn(1.35), chan.bass]);
