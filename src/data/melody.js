/* 선율 라이브러리 — 16 · 32 · 64마디
   ...
   ⚠ **이 파일의 일부는 기계가 써 넣습니다.** `genres/profiles/_apply*.mjs` 가
   `MELODY_KIT_PRESET` · `RIFF_KIT_PRESET` · `BLINE_KIT_PRESET` 세 표에 프리셋별
   배정을 끼워 넣습니다. 끼우는 자리는 `melodyPoolFor` · `riffPoolFor` ·
   `blinePoolFor` **함수 선언 바로 앞의 `};`** 입니다.
   그러니 그 세 함수의 **이름을 바꾸거나 위치를 옮기면 생성기가 멈춥니다.**
   (2026-09-20 까지는 앵커가 함수 위의 한글 주석 문장이어서 더 위험했습니다 —
   주석을 다듬기만 해도 깨졌습니다. 코드 식별자로 옮겼습니다.)
   손으로 고쳐도 되지만, 배치표를 다시 돌리면 덮어쓰지 않고 **중복을 건너뛰는**
   방식이라 손으로 넣은 줄이 남습니다. 근거는 `genres/profiles/*.json` 에 있고
   둘이 어긋나면 `check-melody-profile.mjs` 가 잡습니다. */
'use strict';
