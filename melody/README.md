# 멜로디 자료 — melody/

> **번호 체계가 `genres/`·`patterns/` 와 다릅니다 — 어긋난 것이 아니라 두 구역입니다.**
>
> | 구역 | 번호 | 무엇 |
> |---|---|---|
> | 이론 | `00`~`02` | 계열을 가로지르는 분석·화성·선율 작법. **코드가 근거로 인용합니다** — `src/data/melody.js` · `harmony.js` · `tools/ci/check-melody-profile.mjs` · 계측 하네스가 절 번호까지 가리킵니다 |
> | 계열 | `10`~`20` | A~K 계열별 선율. `genres/`·`patterns/` 의 번호에 **9를 더한 값**입니다 |
>
> 계열 짝: `genres/01-rock` ↔ `melody/10-rock` · `02-pop` ↔ `11-pop` ·
> `03-hiphop` ↔ `12` · `04-rnb-soul-funk` ↔ `13` · `05-electronic` ↔ `14` ·
> `06-jazz` ↔ `15` · `07-roots` ↔ `16` · `08-latin` ↔ `17` · `09-caribbean` ↔ `18` ·
> `10-african` ↔ `19` · `11-regional` ↔ `20`.
>
> 번호를 `genres/` 와 나란히 맞추자는 이야기가 나올 수 있는데, **하지 않는 편이
> 낫습니다.** 계열 파일을 `01`~`11` 로 옮기면 이론 파일 `01-harmony.md` ·
> `02-melody-theory.md` 와 이름이 부딪히고, 그 둘은 코드와 검사에서 스무 곳 넘게
> 인용됩니다. 번호가 나란해 보이는 이득을 위해 **코드가 의존하는 것을 건드리는**
> 거래가 됩니다. 계열 파일을 가리키는 링크는 지금 한 건도 없습니다.

`genres/` 가 **악기·음색**, `patterns/` 가 **박자**를 다룹니다.
여기는 **선율**만 다룹니다.

| 폴더 | 다루는 것 |
|---|---|
| [../genres/](../genres/) | 악기 · 음색 · 수치 |
| [../patterns/](../patterns/) | 드럼 박자 · 화성 아키타입(1마디) |
| **melody/** | **16마디 선율 — 음역 · 리듬 · 윤곽 · 도수 관용구** |

---

## 1. 왜 16마디인가

기존 `patterns/00-harmony.md` 의 화성 아키타입은 **1마디**입니다.
1마디를 반복하면 그루브는 되지만 선율은 안 됩니다. 선율은
"긴장을 쌓고 푸는" 구조라 최소 8마디, 보통 16마디가 한 덩어리입니다.

16마디를 쓰는 이유는 대중음악의 절(verse)·후렴(chorus)이 대개
8마디 또는 16마디이기 때문입니다. 8마디 두 번으로 읽어도 됩니다.

---

## 2. 표기

`patterns/README.md` §1 의 건반 트랙 표기를 그대로 씁니다.

| 기호 | 뜻 |
|---|---|
| `a`~`h` | **단음** — 스케일의 0~7도 |
| `0`~`7` | **3화음** — 그 도수를 근음으로 (인덱스 r, r+2, r+4) |
| `-` | 쉼 |

선율은 단음(`a`~`h`)이 기본입니다. 화음은 강조하고 싶은 자리에만 씁니다.

**도수는 절대 음이 아닙니다.** 근음(Root)과 스케일 노브를 따라갑니다.
같은 선율이 Minor Pentatonic 에서는 5음, Natural Minor 에서는 7음으로 읽힙니다.

```
스케일별 a~h 가 가리키는 반음
  Minor Pentatonic : 0  3  5  7 10 12 15 17
  Natural Minor    : 0  2  3  5  7  8 10 12
  Dorian           : 0  2  3  5  7  9 10 12
  Major            : 0  2  4  5  7  9 11 12
```

---

## 3. 원칙

선율을 지어낼 때 지킨 것들입니다.

1. **도약 뒤에는 순차진행.** 4도 이상 뛰면 그 다음은 반대 방향으로 한 도수씩.
   이게 없으면 아르페지오지 선율이 아닙니다.
2. **쉼이 음만큼 중요합니다.** 16스텝을 다 채우면 선율이 아니라 시퀀스가 됩니다.
   장르별로 밀도를 다르게 잡았습니다(§ [00-analysis.md](00-analysis.md) 표).
3. **반복 + 변형.** 1~4마디를 5~8마디에서 끝만 바꿔 되풀이합니다.
   완전히 새로운 4마디를 네 번 이으면 기억에 안 남습니다.
4. **마지막 마디는 비웁니다.** 다음 덩어리로 넘어갈 자리를 남기고,
   필인(드럼)이 그 자리를 채웁니다.
5. **음역은 keys 트랙의 옥타브 노브를 따릅니다.** 도수만 정하므로
   장르별 음역은 `TONE_KIT` 의 옥타브가 아니라 사용자가 정합니다.

---

## 4. 파일

번호가 두 덩어리입니다. **00~02 는 공통 이론**, **10~20 은 계열별**입니다.
계열별 파일은 `../genres/` 의 A~K 순서를 그대로 따릅니다.

### 공통

| 파일 | 내용 |
|---|---|
| [00-analysis.md](00-analysis.md) | 장르별 선율 특성 — 밀도 · 윤곽 · 관용구 · 그리드의 한계 |
| [01-harmony.md](01-harmony.md) | 화성학 기초 — 도수 · 화음 · 기능 · 진행 · 케이던스 |
| [02-melody-theory.md](02-melody-theory.md) | 선율 작법 — 윤곽 · 도약 · 정점 · 프레이즈 · **지표 ①~⑫** |

### 계열별

각 파일이 **음정 어휘 · 리듬 어휘 · 악구 형식 · 기타 리프 · 베이스 라인 ·
코드 키 매핑 · 지표 예외 · 레퍼런스**를 같은 순서로 담습니다.

| 파일 | 계열 | 짝이 되는 장르 문서 |
|---|---|---|
| [10-rock.md](10-rock.md) | A. Rock | [../genres/01-rock.md](../genres/01-rock.md) |
| [11-pop.md](11-pop.md) | B. Pop | [../genres/02-pop.md](../genres/02-pop.md) |
| [12-hiphop.md](12-hiphop.md) | C. Hip Hop | [../genres/03-hiphop.md](../genres/03-hiphop.md) |
| [13-rnb-soul-funk.md](13-rnb-soul-funk.md) | D. R&B · Soul · Funk | [../genres/04-rnb-soul-funk.md](../genres/04-rnb-soul-funk.md) |
| [14-electronic.md](14-electronic.md) | E. Electronic | [../genres/05-electronic.md](../genres/05-electronic.md) |
| [15-jazz.md](15-jazz.md) | F. Jazz | [../genres/06-jazz.md](../genres/06-jazz.md) |
| [16-roots.md](16-roots.md) | G. Blues · Country · Folk | [../genres/07-roots.md](../genres/07-roots.md) |
| [17-latin.md](17-latin.md) | H. Latin | [../genres/08-latin.md](../genres/08-latin.md) |
| [18-caribbean.md](18-caribbean.md) | I. Caribbean | [../genres/09-caribbean.md](../genres/09-caribbean.md) |
| [19-african.md](19-african.md) | J. African | [../genres/10-african.md](../genres/10-african.md) |
| [20-regional.md](20-regional.md) | K. 기타 지역 | [../genres/11-regional.md](../genres/11-regional.md) |

### 데이터

| 파일 | 내용 |
|---|---|
| `../src/data/melody.js` | PHRASE · FORM · MELODY · RIFF · BLINE 과 각 KIT 표 |
| `../src/data/preset-index.js` | PRESET_SUB · PRESET_CAT — 어느 프리셋이 어느 계열인지 |
| `../tools/analyze-melody.html` | §9 지표로 데이터를 재는 검사 하네스 |

---

## 4-1. 16마디 밖 — 32 · 64루프

`melody.js` 는 16마디 선율마다 **긴 판**을 함께 만듭니다.
키 이름은 `프레이즈쌍_l32` · `프레이즈쌍_l64` 입니다 (예: `rock_l32`).

```
LONG_FORMS = { 32 :[['AABA',0],['AABB',1]],
               64 :[['AABA',0],['AABB',1],['ABAB',2,'invert'],['AABA',0]],
               '64h':[['AAAB',0],['AABA',0],['AAAB',1],['AAAB',0]] }
```

두 번째 값은 **모방진행(sequence)의 올림 단수**입니다.
16마디 덩어리를 그냥 두 번 트는 것과 다른 점이 셋입니다.

1. 덩어리마다 **폼이 바뀝니다** — 전개가 달라집니다.
2. 두 번째 덩어리부터는 도수를 통째로 올려 **새 재료**를 만듭니다.
3. 이어지는 자리의 종지를 **열어 둡니다**(`openEnd()` — 끝의 `a`→`b`,
   `0`→`1`). 마지막 덩어리만 닫습니다.

### 64루프는 이제 34개 재료 **전부**에 붙습니다 (2026-09-17)

예전에는 여덟 개뿐이었습니다 — `LONG_64 = ['rock','pop','bal','cin','ant',
'jazz','amb','root']`. 이유는 «힙합·트랩처럼 두세 음 훅이 정체성인 계열에
64마디를 물리면 같은 두 음을 64번 듣게 된다» 였고, 그것은 **전개형 plan(`64`)을
물렸을 때** 맞는 말입니다. 그 plan 은 덩어리마다 도수를 한 단·두 단 올리므로
훅이 훅이 아니게 됩니다.

그래서 훅을 **제자리에 두고 전개 순서만 바꾸는** plan(`'64h'`)을 따로 두었습니다.
셋째 덩어리(브릿지 자리)만 한 단 올리고 나머지 셋은 도수를 안 건드립니다 —
제시(AAAB) → 순서 바꾸기(AABA) → 한 단 올린 브릿지(AAAB+1) → 재현(AAAB).
이 계열이 실제로 64마디를 버티는 방법은 선율 전개가 아니라 **편성 변화**이고,
그쪽은 곡 구조(`src/seq/arrange.js`)가 맡습니다.

| plan | 목록 | 재료 |
|---|---|---|
| 전개형 `64` | `LONG_64` 19개 | 록·팝·발라드·시네마틱·앤섬·재즈·앰비언트·루츠·블루스·가스펠·월드·성가·보사·펑크와 교차분 |
| 훅형 `64h` | `LONG_64H` 15개 | 힙합·트랩·EDM·칩튠·디스코·개러지·래가·아프로·아프로댄스홀·라틴·카리브·파워코드와 교차분 |

이름은 양쪽 다 `_l64` 입니다 — 길이 선택이 접미사로 찾으므로 여기서 이름을
가르면 훅형 재료만 64루프를 못 고르게 됩니다. 라벨로만 구분합니다(«64루프(훅 유지)»).
**16·32루프는 한 글자도 안 건드렸습니다.**

### 리프·베이스도 같은 생성기를 씁니다

`buildLong()` 이 `PHRASE`(건반 프레이즈 표)를 하드코딩하고 있어서 기타 리프와
베이스는 16마디에 묶여 있었습니다. 64마디 선율 밑에서 기타만 같은 16마디를 네 번
되풀이하니 «길어진» 것이 아니라 «늘어진» 소리였습니다. 표를 인자로 받는
`buildLongP(pa, pb, plan)` 으로 고쳐 `RIFF_PHRASE`·`BASS_PHRASE` 도 같이 씁니다
(생성기가 보는 것은 «마디 문자열» 뿐이라 표기 계열이 달라도 그대로 돕니다).

긴 판은 **재료쌍마다 하나씩**입니다. 폼(AABA·AABB…)은 16마디를 잇는 방식이고
긴 판은 자기 plan 이 폼을 정하므로, 같은 쌍의 세 폼에서 긴 판을 따로 만들면
내용이 똑같은 항목이 셋 생깁니다. 대신 원본 항목에 **연결**(`l32`·`l64`)을 걸어
둡니다.

### 길이 선호

`melodyLenPool(pool, pref)` 이 선율 풀을 갈아 끼웁니다. `'auto'` 는 16루프와
긴 것을 섞고, `'32'`·`'64'` 는 같은 재료의 긴 판이 있으면 그쪽으로 갑니다 —
없으면 16루프를 그대로 둡니다. **앱 기본값은 `'64'`** 입니다.

> ⚠ **곡 구조가 켜져 있으면 폼이 이깁니다** (2026-09-17). 폼마다 총 마디가
> 다르고(32·48·64·96·128·224) 선율이 그것을 나누지 못하면 코러스가 선율의
> 브릿지 위에 얹힙니다. 12마디 블루스 네 바퀴(48마디)에 64마디 선율을 물리면
> 둘이 192마디마다 한 번 만납니다. `lenPref()`(sequencer.js)가 `formMelLen()`
> 을 먼저 보고, 폼이 없을 때만 사용자 선택을 씁니다.
> **형식이 음악의 사실이고 선율 길이는 우리가 만든 눈금입니다** —
> [genres/00-form.md](../genres/00-form.md) §2.

리프·베이스는 `riffLenPool`·`blineLenPool` 이 맡습니다. 이쪽은 **이름으로 형제를
찾지 않습니다** — 리프 이름(`rock_power`)은 폼 접미사가 아니라 별명이라 접미사
치환이 엉뚱한 이름을 만듭니다. 대신 위의 연결(`l32`·`l64`)을 따라갑니다.

> 이 불변식들은 `tools/ci/check-song-length.mjs` 가 지킵니다 —
> 재료마다 64마디판이 있는가(D) · 길이 64 를 골랐을 때 프리셋 357종이 전부
> 64마디를 받는가(E).

> **지표 ⑥(서로 다른 마디)은 긴 폼에 그대로 대면 안 됩니다.**
> 16마디를 전제로 잡은 값이라 32루프 0.38, 64루프 0.20~0.33 으로 내려갑니다.
> 64루프의 마지막 16마디는 **재현부**라 첫 덩어리를 되풀이하는 것이 형식입니다.
> ([02-melody-theory.md](02-melody-theory.md) §10 과 같은 성격의 예외입니다.)

---

## 5. 계열 이름은 지어내지 않습니다

계열(A~K)과 하위분기 이름은 **`../genres/00-tree.md` 와
`../src/data/preset-index.js` 의 `PRESET_CAT`·`PRESET_SUB` 에 실재하는 것만**
씁니다. 문서에서 새 계열명을 만들면 코드와 어긋나 곧 거짓말이 됩니다.

마찬가지로 계열별 문서가 인용하는 `PHRASE`·`MELODY`·`RIFF`·`BLINE` 키는
전부 `melody.js` 에서 확인한 것입니다.

---

## 6. 알려진 구멍 — `'pop_arch'` → ✅ **닫혔습니다 (2026-09-14)**

> **상태**: 두 번에 걸쳐 고쳐졌습니다.
>
> 1. `'pop_arch'` 는 키가 아니라 라벨('팝 아치')이었고, 실제 키는 `pop_aabb`
>    입니다. 폴백이 없는 키를 가리키던 것은 그때 고쳐졌습니다 —
>    `melody.js` 의 `melodyPoolFor()` 주석에 경위가 있습니다.
> 2. 그래도 **폴백으로 떨어지는 프리셋 자체가 남아 있었습니다.** 아래 본문이
>    「69개」라고 적은 것이 그것입니다. 원인은 `PRESET_CAT` 이 357종 중
>    **56종만** 손으로 적힌 예외표인데 계열 판정을 그것에만 맡긴 것이었습니다.
>    프리셋은 raw 정의에 `cat:'A'~'K'` 를 **직접 적어 두므로**(`LIB[name].cat`)
>    그쪽을 먼저 보면 됩니다 — `ui/build.js` 의 `catOf()` 와
>    `seq/arrange.js` 의 `catFor()` 가 이미 그렇게 하고 있었고,
>    선율·리프·베이스 배정 함수만 뒤처져 있었습니다.
>    `melody.js` 에 `poolCatFor()` 를 두어 셋 다 같은 우선순위를 보게 했습니다.
>
> **아래 본문이 「깨지는 것은 선율 쪽뿐」이라고 적은 것은 틀렸습니다.**
> 리프·베이스는 폴백 키가 실재해서 *죽지 않았을* 뿐, 실제로는 훨씬 넓게
> 어긋나 있었습니다. 고치기 전후 실측:
>
> | 폴백으로 떨어지던 프리셋 | 전 | 후 |
> |---|---|---|
> | 선율 `MELODY` | 69종 (19%) | **0종** |
> | 기타 리프 `RIFF` | **140종 (39%)** | **0종** |
> | 베이스 라인 `BLINE` | **152종 (43%)** | **0종** |
>
> (남은 2종은 `Play Along`·`Casio Cells` — 계열 `X` 인 도구용 프리셋이라
> 폴백이 맞습니다.)
>
> 눈에 띄는 결과: 댄스홀 6종이 팝 아치 선율 + **록 파워코드 리프** +
> **록 베이스라인**을 받고 있다가, 이제 `car_*`(카리브) 선율 · `skank`
> (레게 스킹크) 리프 · `breg_*`(레게 베이스)를 받습니다.
>
> 아래 본문은 «무엇이 문제였는지» 의 기록으로 남깁니다.

`melodyPoolFor()` 의 마지막 fallback 이 `['pop_arch']` 인데
**`MELODY` 에 그런 키가 없습니다.**

```js
function melodyPoolFor(name){
  const sub=MELODY_KIT[PRESET_SUB[name]];
  if(sub) return sub;
  return MELODY_KIT_CAT[PRESET_CAT[name]] || ['pop_arch'];   // ← 없는 키
}
```

`PRESET_SUB` 의 355개 프리셋 중 **69개**가 여기로 떨어집니다 —
하위분기가 `MELODY_KIT` 에 없고 프리셋 이름도 `PRESET_CAT` 에 없는 경우입니다.
어느 하위분기가 비어 있는지는 각 계열 문서의 **"하위분기가 비어 있는 자리"**
절에 적었습니다.

`riffPoolFor()`·`blinePoolFor()` 의 fallback(`'rock_power'`·`'brock_aaba'`)은
실재하는 키라 문제가 없습니다 — 깨지는 것은 선율 쪽뿐입니다.

> 이 문서는 상태를 기록할 뿐 데이터를 고치지 않습니다.
