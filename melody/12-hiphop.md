...
| 64루프 | `hip_l64` · `trp_l64` (훅형) · `jazz_l64` · `amb_l64` · `cin_l64` (전개형) |

> **`hip_l64` · `trp_l64` 가 생겼습니다 (2026-09-17).** 예전에는 없었고 그 이유는
> "두세 음 훅이 정체성인 계열에 64마디를 물리면 같은 두 음을 64번 듣는 꼴" 이었습니다.
> 맞는 말이지만 그것은 **전개형 plan** 의 이야기입니다 — 덩어리마다 도수를 올리면
> 훅이 훅이 아니게 됩니다. 힙합·트랩에는 훅을 제자리에 두고 전개 순서만 바꾸는
> `'64h'` plan 을 겁니다(melody/README.md §4-1). 길이는 선율 전개가 아니라
> **편성 변화**로 버는 것이 이 계열의 방식이고, 그쪽은 곡 구조가 맡습니다.
