...
| 64루프 | `edm_l64` · `chip_l64` · `dis_l64` · `trp_l64` (훅형) · `amb_l64` · `bal_l64` · `jazz_l64` · `cin_l64` (전개형) |

> **`edm_l64` · `chip_l64` 가 생겼습니다 (2026-09-17).** 16분 아르페지오를
> 전개형 plan 으로 64마디 물리면 «변화 없이 길기만» 한 것이 맞습니다 — 그래서
> 이쪽에는 훅을 제자리에 두는 `'64h'` plan 을 겁니다(melody/README.md §4-1).
> 도수를 올려 가는 긴 전개가 필요하면 여전히 `cin_l64`(시네마틱 상승)가 맞습니다.
