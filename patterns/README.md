...
**세기 세 층** — 엔진이 실제로 구분하는 값입니다 (`src/data/pattern-codec.js`).

| 글자 | 세기 | 무엇 |
|---|---|---|
| `X` | 1.00 | 강세 — 박머리, 백비트 |
| `x` | 0.60 | 보통 |
| `o` | 0.25 | **고스트** — 스틱이 스치는 정도 |

고스트는 «작게 친 보통 타»가 아닙니다. 세기가 음량뿐 아니라 **로우패스
컷오프·감쇠시간**까지 바꾸므로(`src/audio/voice-drum.js` §9), `o` 는 어둡고
짧게 울립니다. 그래서 `x` 를 볼륨만 낮춰서는 같은 소리가 나오지 않습니다.

> ⚠ **16분 트랙을 전부 `x` 로 적지 마십시오.** 음량이 같은 16타는 그루브가
> 아니라 «기계가 떠는 소리»가 됩니다. 박머리는 `X`, 사이는 `o` 로 갈라야
> 합니다. 특히 Funk·Neo-Soul·Boom Bap 처럼 **고스트가 장르의 정체성인**
> 계열에서 이 구분이 없으면 장르가 성립하지 않습니다.
...
### 골격 공유에 대해 — **2026-08-17 에 끝났습니다**

여기에는 «계열 파일은 골격이 같은 장르를 한 블록에 묶어 보여준다» 고 적혀
있었습니다. 파생 프리셋이 원형을 상속한 결과였고, 실제로 이랬습니다:

| | 그때 | 지금 |
|---|---|---|
| 박자가 완전히 같은 무리 | **26무리 318종** | **0** |
| 록 49종의 서로 다른 골격 | 7개 | 49개 |
| 형제와 편성이 한 칸도 안 다른 프리셋 | 207종 | 0 |

록 55종이 킥·스네어·햇·베이스가 한 글자도 안 달랐고, 일렉트로닉 54종이
또 그랬습니다. Deep House 와 Hard Techno 와 Frenchcore 가 같은 패턴이었습니다.

**상속 자체를 끊었습니다**([`_build.js`](../src/data/presets/_build.js)) —
계통도의 상하위 관계는 **논리적 분류로 남기고**, 소리를 만드는 값은
프리셋마다 직접 적습니다. 계열 파일의 블록은 이제 **장르 하나에 하나**입니다.

> 이름 앞의 `·` 표기는 «규칙 파생» 을 뜻했는데, 파생이 없어졌으므로
> 계열 파일에서 뺐습니다. `genres/*.md` 의 설정값 표에는 아직 남아 있습니다 —
> 그쪽은 «청감으로 검증했는가» 를 표시하는 다른 뜻입니다.
