...
> 앱의 «지금 장르» 줄과 근거 패널이 이 표와 계열 파일(`01-rock.md` …)의
> 레퍼런스 표를 그대로 보여 줍니다 — `tools/build-refdata.mjs` 가
> `src/data/references.js` 로 굳힙니다. **표를 고쳤으면 그것을 다시 돌리십시오.**
> 같은 프리셋이 여러 파일에 있으면 계열 파일이 먼저이고, 아티스트·앨범은 합칩니다.
>
