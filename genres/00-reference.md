...
> ## ⚠ 이 문서는 2026-08 의 작업 일지입니다 — 현행 자료가 아닙니다
>
> **2026-09-20 부터 `tools/build-refdata.mjs` 가 이 파일을 읽지 않습니다.**
> 앱의 근거 패널에 나오는 레퍼런스는 전부 **계열 파일**(`01-rock.md` … `11-regional.md`)
> 에서 옵니다.
>
> 왜 뺐는가. 여기 105행이 **전부 계열 파일에도 있고**, 이 문서에만 있는 프리셋은
> **0종**이었습니다. 그런데 빌드가 계열 파일을 먼저 쓰므로, 문구가 갈라진 **85행은
> 화면에 영원히 안 나왔습니다.** 고쳐도 아무 일도 일어나지 않는 글을 데이터 원본
> 자리에 두는 것이 가장 나쁜 상태였습니다.
>
> 게다가 내용이 낡았습니다. 여기 Heavy Metal 은 「갤럽」인데, 계열 파일은 대표곡
> 교정으로 **「곧은 8분(갤럽 아님)」** 입니다 — 갤럽은 NWOBHM 으로 옮겨 갔습니다.
> 즉 **반증된 서술**이 데이터로 읽히고 있었습니다.
>
> 빼기 전에 이 문서에만 있던 것은 계열 파일로 옮겼습니다 — Deftones(Nu Metal) ·
> The Ventures(Surf Rock) · `A.C. Jobim` → `Antônio Carlos Jobim` · 괄호 주석 둘
> (Phonk · Synthwave). `Howlin Wolf` 와 `Howlin' Wolf` 는 빌드의 `nameKey()` 가
> 원래 합치던 것이고, Garage Rock 의 The Stooges 는 **오귀속**이라 옮기지 않았습니다
> (계열 파일의 Proto-punk 행에 이미 있고, 대표곡 표에 웹으로 확인한 곡이 둘 있습니다).
>
> **그래서 이 파일을 고쳐도 앱은 바뀌지 않습니다.** 레퍼런스를 고치려면 계열 파일을
> 고치십시오. 이 글은 당시의 판단 근거를 남겨 두려고 보존합니다.
...
> ~~앱의 «지금 장르» 줄과 근거 패널이 이 표와 계열 파일의 레퍼런스 표를 그대로
> 보여 줍니다.~~ **2026-09-20 부터 이 문서는 해당하지 않습니다** — 위 배너를
> 보십시오. 화면에 나오는 것은 계열 파일(`01-rock.md` …)의 레퍼런스 표뿐이고,
> `tools/build-refdata.mjs` 가 그것을 `src/data/references.js` 로 굳힙니다.
> **계열 파일의 표를 고쳤으면 그것을 다시 돌리십시오.**
>
> **대표곡은 이 문서가 아니라 계열 파일의 «대표곡» 표에 있습니다**(2026-09-16, `01-rock.md` 부터).
> 그쪽은 곡이 실재하고 출처가 그 장르로 분류했는지를 웹에서 확인한 것만 적은 표라,
> 기억에 의존한 이 문서의 앨범명과 신뢰도가 다릅니다.
