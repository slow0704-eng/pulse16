...
> **Afrobeat(단수)은 프리셋이 없습니다** — 이 도구가 가진 것은 Afrobeats(복수)뿐입니다(§0).
> 그래서 아래 표에 줄을 두지 않습니다. 이 표의 계약은 «프리셋 → 레퍼런스» 라,
> 프리셋이 없는 줄을 넣으면 빌드가 매번 「프리셋 이름에 닿지 않는 표 이름」 경고를
> 냅니다 — 늘 나오는 경고는 아무도 안 보게 되고, 그러면 진짜 오타가 묻힙니다.
> 펠라 쿠티 계보의 편성(**혼 섹션** · 커팅 기타 · 콩가)은 §0 비교표와 §1 에 있습니다.
